...
arg_parse.add_argument('--topk', type = int, default = 5, help = 'classes with highest probability')
arg_parse.add_argument('--category_names', type = str, default = 'cat_to_name.json', help = 'categories to names')
arg_parse.add_argument('--checkpoint', action='store', default='checkpoint.pth')
arg_parse.add_argument('--gpu', type = str, action = 'store_true', help = 'needed for training the network')
...
def load_chkpnt(path):
    ...
    elif torch.cuda.is_available() and args.gpu :
        map_loc= lambda storage, location: storage.cuda() 
